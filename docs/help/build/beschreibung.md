...
Ember Media Manager wird als Visual-Studio-Solution (`Ember Media Manager.sln`) gebaut. Alle rund 30 Projekte der Solution — Hauptanwendung, `EmberAPI`, `KodiAPI`, `Trakttv` und alle Addon-Module — zielen einheitlich auf **.NET Framework 4.8.1**.

Nach einem frischen `git clone` genügen zwei Schritte, um die vollständige Solution zu kompilieren: NuGet-Pakete wiederherstellen und die Solution mit MSBuild bauen. Es ist weder ein separates .NET Framework 4.8.1 Developer Pack noch ein externer Checkout der TVDB-Bibliothek erforderlich — beides liegt im Repository bzw. wird per NuGet bezogen.
