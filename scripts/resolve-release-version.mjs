// Generalized from FinanceManager's scripts/resolve-release-version.mjs (see
// ci-target-schema.md section 4.10), with all FinanceManager-specific asset
...
const AUTOMATIC_RELEASE_BRANCHES = ["master"];
...
// Softwareschmiede publishes a single win-x64 asset (release.zip) plus the update
// manifest (update.json), unlike FinanceManager/VideoPlayer/Rezepte which publish
// per-runtime-identifier archives - see build-and-package/action.yml for why the asset
// name intentionally is not platform-suffixed.
export function expectedReleaseAssetNames() {
  return ["release.zip", "update.json"];
}
